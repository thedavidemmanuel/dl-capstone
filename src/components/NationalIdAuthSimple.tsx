...
export default function NationalIdAuth({ onSuccess, onBack, isLoading = false }: NationalIdAuthProps) {
  ...
  const handleIdSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!authResult.success) {
        ...
        setError(authResult.message || 'National ID verification failed');
        return;
      }
      
      // SUCCESS - Switch to OTP screen
      console.log('✅ Auth initiated successfully, transaction ID:', authResult.transactionId);
      setTransactionId(authResult.transactionId!);
      setStep('otp-verification');
      setSuccess(authResult.message || 'OTP sent to your registered phone number');
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const verifyResult = await supabaseAuthService.verifyOtp(nationalId, otp, transactionId);
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="mt-6 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-blue-800 text-xs text-center">
            <strong>Development Mode:</strong> Using Supabase directly. Use any 13-digit National ID from the database and any 6-digit OTP for testing.
          </p>
        </div>
      )}
    </div>
